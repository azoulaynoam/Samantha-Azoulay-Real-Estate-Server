    ...
    if(body.images && body.images !== null) {
        ...
        if(Array.isArray(body.images)){
            body.images.forEach(image => {
                if(Array.isArray(property.images) && property.images.some(img => img.path == image)){
                    images.push({'path': image})
                }
            })
        }
        else if(Array.isArray(property.images) && property.images.some(img => img.path == body.images)){
...
function create_property_body(body) {
    let fields = ['action', 'status', 'free_text_en', 'free_text_he', 'rooms', 'bedrooms', 'bathrooms', 'size', 'price']
    let new_property = {}
    Object.keys(body).forEach(key => {
        if(fields.includes(key))
            new_property[key] = body[key]
    });
    return new_property
}
...
async function get_property(req, property){
    ...
    if (old_images && old_images !== null){ // Adding old images from body.
        if (images && images !== null){
            old_images.forEach(img => {
                if(!images.some(image => image.path === img.path))
                    images.push(img)
            })
        } else {
            images = old_images
    ...
    if (old_images && old_images.length){ // Deleting unnecessarry files from previus version of property
...
exports.update_property = async function(req, res) {
    ...
    if(!property || property === null){
        ...
        Property.findOneAndUpdate({id: req.params.propertyId}, new_property, {new: true, runValidators: true}, function(err, property){
            if(err)
                res.send(err)
            else
                res.json(property)
...
exports.delete_property = function(req, res) {
    Property.findOneAndDelete({id: req.params.propertyId}, function(err, property){
...
exports.list_properties = function(req, res) {
    let fields = ['id', 'action', 'status', 'free_text_en', 'free_text_he', 'rooms', 'bedrooms', 'bathrooms', 'size', 'price']
    let filter = {}
    let field = null
    let order = null
    let start = null
    ...
    try{
        Object.keys(req.query).forEach(param => {
            switch(param) {
                case '_sort':
                    if (fields.includes(req.query[param]))
                        field = req.query[param]
                    break
                case '_order':
    ...
    Property.find(filter, function(err, properties) {
        ...
        if(err){
            ...
            if(start || end){
                ...
                res.header('Content-Range', 'properties ' + start + '-' + (end - 1) + '/' + len)
            }
